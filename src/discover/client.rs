...
/// Client for discovering Kubernetes API resources.
...
impl DiscoverClient {
    ...
    /// Lists all API resources available in the Kubernetes cluster.
    /// e.g. `pods`, `deployments`, `services`
    ...
    /// Lists API resources from all API groups
    /// e.g. `apps/v1`, `batch/v1`, `networking.k8s.io/v1`
    ...
    /// Lists API resources from the core API group
    /// e.g. `v1` (which includes `pods`, `services`)
